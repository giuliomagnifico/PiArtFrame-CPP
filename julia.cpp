...
void JuliaSet::Render(UWORD xResolution, UWORD yResolution) {
  ...
  const int minBlackPixelCount = totalPixelCount * 0.05;
  const int maxBlackPixelCount = totalPixelCount * 0.85;
  ...
  while (!validImage) {
    ...
    if (blackPixelCount > maxBlackPixelCount) {

      this -> x += 0.1 * this -> w * (rand() % 2 == 0 ? -1 : 1);
      this -> y += 0.1 * this -> h * (rand() % 2 == 0 ? -1 : 1);
    } else if (blackPixelCount < minBlackPixelCount) {

      this -> x += 0.05 * this -> w * (rand() % 2 == 0 ? -1 : 1);
      this -> y += 0.05 * this -> h * (rand() % 2 == 0 ? -1 : 1);
    }

    this -> y += panFactor * (rand() % 2 == 0 ? -1 : 1) * this -> h;
    ...
    if (blackPixelCount > maxBlackPixelCount) {
      ...
      if (blackPixelCount > maxBlackPixelCount || blackPixelCount < minBlackPixelCount) {
...
bool JuliaSet::IsJuliaPoint(double fX, double fY, int iterations) {
  ...
  std::complex < double > c(-0.8, 0.145);
